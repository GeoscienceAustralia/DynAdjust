...
#include <dynadjust/dnaimport/dnainterop.hpp>  // Now explicitly included
...
void PrintOutputFileHeaderInfo(std::ofstream* f_out, const std::string& out_file, project_settings* p,
                               const std::string& header) {
    // Print formatted header
    print_file_header(*f_out, header);

    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "File name:" << safe_absolute_path(out_file) << std::endl
           << std::endl;
    ...
    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Input folder: " << safe_absolute_path(p->g.input_folder)
           << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Output folder: " << safe_absolute_path(p->g.output_folder)
           << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Stations file:" << safe_absolute_path(p->i.bst_file)
           << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Measurements file:" << safe_absolute_path(p->i.bms_file)
           << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Associated station file:" << safe_absolute_path(p->i.asl_file)
           << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left
           << "Associated measurement file:" << safe_absolute_path(p->i.aml_file) << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left
           << "Duplicate stations output file:" << safe_absolute_path(p->i.dst_file) << std::endl;
    *f_out << std::setw(PRINT_VAR_PAD) << std::left
           << "Similar measurement output file:" << safe_absolute_path(p->i.dms_file) << std::endl;
    ...
    if (p->i.user_supplied_frame)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Project reference frame:" << p->i.reference_frame
               << " (user supplied)" << std::endl;
    else
        *f_out << std::setw(PRINT_VAR_PAD) << std::left
               << "Project reference frame:" << "To be adopted from the first input file" << std::endl;

    if (p->i.override_input_rfame)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left
               << "Override input file ref frame:" << yesno_string(p->i.override_input_rfame) << std::endl;
    ...
    if (isEpsgDatumStatic(epsgCode) && p->i.user_supplied_frame)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Project epoch:" << p->i.epoch
               << " (adopted reference epoch of " << p->i.reference_frame << ")" << std::endl;
    else {
        if (p->i.user_supplied_epoch)
            *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Project epoch:" << p->i.epoch << " (user supplied)"
                   << std::endl;
        else
            *f_out << std::setw(PRINT_VAR_PAD) << std::left
                   << "Project epoch:" << "To be adopted from the first input file" << std::endl;
    }

    if (!p->i.include_msrs.empty())
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Strip all measurements except:" << p->i.include_msrs
               << std::endl;
    ...
    if (p->i.search_nearby_stn)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left
               << "Search for nearby stations:" << "tolerance = " << p->i.search_stn_radius << "m" << std::endl;
    ...
    if (p->i.search_similar_msr_gx)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Search for similar GNSS measurements:" << "yes"
               << std::endl;
    ...
    if (!p->i.bounding_box.empty()) {
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Bounding box: " << p->i.bounding_box << std::endl;
        if (p->i.split_clusters)
            *f_out << std::setw(PRINT_VAR_PAD) << std::left
                   << "Split GNSS clusters: " << (p->i.split_clusters ? "Yes" : "No") << std::endl;
    } else {
        if (!p->i.stn_associated_msr_include.empty())
            *f_out << std::setw(PRINT_VAR_PAD) << std::left
                   << "Stations to include: " << p->i.stn_associated_msr_include << std::endl;
        if (!p->i.stn_associated_msr_exclude.empty())
            *f_out << std::setw(PRINT_VAR_PAD) << std::left
                   << "Stations to exclude: " << p->i.stn_associated_msr_exclude << std::endl;
    ...
    if (!p->i.seg_file.empty())
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Segmentation file:" << safe_absolute_path(p->i.seg_file)
               << std::endl;

    if (p->i.import_block)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left << "Import stns & msrs from block: " << p->i.import_block_number
               << std::endl;
    else if (p->i.import_network)
        *f_out << std::setw(PRINT_VAR_PAD) << std::left
               << "Import stns & msrs from network: " << p->i.import_network_number << std::endl;
...
int ParseCommandLineOptions(const int& argc, char* argv[], const boost::program_options::variables_map& vm,
    ...
    if (vm.count(EPOCH) && !vm.count(REFERENCE_FRAME)) {
        std::cout << std::endl
                  << "- A reference frame must be provided when providing an epoch. " << std::endl
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    for_each(p.i.input_files.begin(), p.i.input_files.end(),
             [&p](std::string& file) { formPath<std::string>(p.g.input_folder, file); });
    ...
    if (p.g.network_name == "network1") {
        ...
        while (true) {
            ...
            if (!std::filesystem::exists(networkASL)) break;
    ...
    if (vm.count(IMPORT_GEO_FILE)) p.i.import_geo_file = 1;
    ...
    //////////////////////////////////////////////////////////////////////////////
    // Ref frame options
    if (vm.count(OVERRIDE_INPUT_FRAME)) p.i.override_input_rfame = 1;
    ...
    if (vm.count(EPOCH)) {
        // Get today's date?
        if (iequals(p.i.epoch, "today")) p.i.epoch = stringFromToday<boost::gregorian::date>();
        ...
        if (p.i.epoch.length() < 10) {
            ...
            if (dateStr.empty()) {
                std::cout << std::endl
                          << "- Error: Cannot parse date '" << p.i.epoch << "'." << std::endl
    ...
    if (vm.count(SPLIT_CLUSTERS)) p.i.split_clusters = 1;

    if (vm.count(IMPORT_SEG_BLOCK) && vm.count(IMPORT_CONTIG_NET)) {
        std::cout << std::endl
                  << "- Error: Cannot import stations and measurements using both options" << std::endl
                  << "  --" << IMPORT_SEG_BLOCK << " and --" << IMPORT_CONTIG_NET << "." << std::endl
                  << "  Please supply only one option." << std::endl
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (vm.count(IMPORT_SEG_BLOCK)) {
        p.i.import_block = 1;
        if (p.i.import_block_number < 1) p.i.import_block_number = 1;
    }

    // Import stations and measurements from a contiguous network
    if (vm.count(IMPORT_CONTIG_NET)) p.i.import_network = 1;
    ...
    if (vm.count(PREFER_X_MSR_AS_G)) p.i.prefer_single_x_as_g = 1;

    if (vm.count(TEST_NEARBY_STNS)) p.i.search_nearby_stn = 1;

    if (vm.count(TEST_SIMILAR_MSRS)) p.i.search_similar_msr = 1;

    if (vm.count(TEST_SIMILAR_GNSS_MSRS)) p.i.search_similar_msr_gx = 1;

    if (vm.count(IGNORE_SIMILAR_MSRS)) p.i.ignore_similar_msr = 1;

    if (vm.count(IGNORE_INSUFFICIENT_MSRS)) p.i.ignore_insufficient_msrs = 1;

    if (vm.count(REMOVE_IGNORED_MSRS)) p.i.remove_ignored_msr = 1;

    if (vm.count(FLAG_UNUSED_STNS)) p.i.flag_unused_stn = 1;

    if (vm.count(TEST_INTEGRITY)) p.i.test_integrity = 1;

    //////////////////////////////////////////////////////////////////////////////
    // GNSS scaling
    if (vm.count(VSCALE) || vm.count(PSCALE) || vm.count(LSCALE) || vm.count(HSCALE) || vm.count(SCALAR_FILE))
        p.i.apply_scaling = 1;
    ...
    if (vm.count(OUTPUT_MSR_TO_STN)) p.o._msr_to_stn = 1;
    ...
    if (vm.count(EXPORT_XML_FILES)) {
        ...
        if (vm.count(EXPORT_SINGLE_XML_FILE)) {
            ...
            p.i.xml_outfile = formPath<std::string>(p.g.output_folder, fileName, "xml");
        }
        // unique files for stations and measurements
        else {
            p.i.export_single_xml_file = 0;
            p.i.xml_stnfile = formPath<std::string>(p.g.output_folder, fileName + "stn", "xml");
            p.i.xml_msrfile = formPath<std::string>(p.g.output_folder, fileName + "msr", "xml");
        }
    }
    ...
    if (vm.count(EXPORT_DNA_FILES)) {
        ...
        p.i.dna_stnfile = formPath<std::string>(p.g.output_folder, fileName, "stn");
        p.i.dna_msrfile = formPath<std::string>(p.g.output_folder, fileName, "msr");
    }

    if (vm.count(EXPORT_ASL_FILE)) p.i.export_asl_file = 1;
    if (vm.count(EXPORT_AML_FILE)) p.i.export_aml_file = 1;
    if (vm.count(EXPORT_MAP_FILE)) p.i.export_map_file = 1;
    if (vm.count(EXPORT_DISCONT_FILE)) p.i.export_discont_file = 1;

    // Simulate
    if (vm.count(SIMULATE_MSR_FILE)) {
        p.i.simulate_measurements = 1;
        p.i.simulate_msrfile = formPath<std::string>(p.g.output_folder, p.g.network_name, "simulated.msr");
    }

    // Station renaming file
    if (vm.count(STATION_RENAMING_FILE)) p.i.rename_stations = 1;

    return EXIT_SUCCESS;
}

void PrintMeasurementstoStations(MsrTally* parsemsrTally, dna_import* parserDynaML, project_settings* p,
                                 pvASLPtr vAssocStnList) {
    ...
    if (p->o._msr_to_stn) {
        ...
        parserDynaML->PrintMeasurementsToStations(p->o._m2s_file, parsemsrTally, p->i.bst_file, p->i.bms_file,
                                                  p->i.aml_file, vAssocStnList);
        ...
        *imp_file << "+ Searching for similar measurements... ";
        ...
        if (!vSimilarMeasurements.empty()) {
            ...
            ss_msg << msr << " measurement" << (msr > 1 ? "s were" : " was")
                   ...
                   << (msr > 1 ? "other measurements" : "another measurement") << ".  See " << p->i.dms_file
                   << " for details." << std::endl;

            *imp_file << ss_msg.str();
            if (!p->g.quiet) std::cout << ss_msg.str();
            ss_msg.str("");
            ...
            // output message
            dms_file << std::endl
                     << "- " << msr << " measurement" << (msr > 1 ? "s were" : " was")
                     << " found to be very similar (if not identical)\n  to "
                     << (msr > 1 ? "other measurements." : "another measurement.  ");

            if (p->i.ignore_similar_msr) dms_file << std::endl << "+ These measurements have been ignored.";
            dms_file << std::endl << std::endl;

            // dump measurements to dms file
            for_each(vSimilarMeasurements.begin(), vSimilarMeasurements.end(),
                     [&dms_file, &comment](dnaMsrPtr& m) { m->WriteDynaMLMsr(&dms_file, comment); });
            ...
            if (p->i.ignore_similar_msr)
                ...
                ss_msg << std::endl
                       ...
                       << "  from the measurement file and re-run " << __BINARY_NAME__ << ", or re-run "
                       << __BINARY_NAME__ << " with the" << std::endl
                       << "  --" << IGNORE_SIMILAR_MSRS << " option.  Alternatively, if each measurement " << std::endl
                       << "  is unique, then call " << __BINARY_NAME__ << " without the --" << TEST_SIMILAR_MSRS
                       << " option." << std::endl
                       << std::endl;
...
void ReportExportedData(const project_settings& p, std::ostream& out, const size_t& file_type, const UINT32& stnCount,
                        const UINT32& msrCount) {
    ...
    if (file_type == dynaml && print_single_xml)
        ...
        if (stnCount > 0) {
            out << "  - " << stn_file;
            if (msrCount > 0) out << std::endl;
        }
        if (msrCount > 0) out << "  - " << msr_file;
        out << "... ";
    }
    out.flush();
}

void ExportStationsandMeasurements(dna_import* parserDynaML, const project_settings& p, std::ofstream* imp_file,
                                   vifm_t* vinput_file_meta, vdnaStnPtr* vstationsTotal, vdnaMsrPtr* vmeasurementsTotal,
    ...
    if ((p.i.export_dynaml || p.i.export_dna_files) && !p.i.override_input_rfame) {
        for (UINT32 i(0); i < vinput_file_meta->size(); ++i) {
            if (!iequals(epsgCode, vinput_file_meta->at(i).epsgCode)) {
                ...
                ssEpsgWarning << std::endl
                              << "- Warning: The default reference frame (" << p.i.reference_frame << ")"
    ...
    if (p.i.export_dynaml && (stnCount > 0 || msrCount > 0)) {
        if (!p.g.quiet) ReportExportedData(p, std::cout, dynaml, stnCount, msrCount);
        ReportExportedData(p, *imp_file, dynaml, stnCount, msrCount);

        if (p.i.export_single_xml_file) {
            // Single output file
            parserDynaML->SerialiseDynaMLfromMemory(vstationsTotal, vmeasurementsTotal, p.i.xml_outfile, p,
                                                    vinput_file_meta, (p.i.flag_unused_stn ? true : false));
        } else {
            // Separate output files (default)
            parserDynaML->SerialiseDynaMLSepfromMemory(vstationsTotal, vmeasurementsTotal, p.i.xml_stnfile,
                                                       p.i.xml_msrfile, p, vinput_file_meta,
    ...
    if (p.i.export_dna_files && (stnCount > 0 || msrCount > 0)) {
        // Separate output files (default)
        if (!p.g.quiet) ReportExportedData(p, std::cout, dna, stnCount, msrCount);
        ReportExportedData(p, *imp_file, dna, stnCount, msrCount);

        parserDynaML->SerialiseDNA(vstationsTotal, vmeasurementsTotal, p.i.dna_stnfile, p.i.dna_msrfile, p,
                                   vinput_file_meta, (p.i.flag_unused_stn ? true : false));
...
int PrepareImportSegmentedData(project_settings& p, bool& userSuppliedSegFile) {
    ...
    if (!std::filesystem::exists(p.i.seg_file)) {
        std::cout << std::endl
                  ...
                  << "         " << p.i.seg_file << std::endl
                  << std::endl
                  << "  Run  'segment " << p.g.network_name << "' to create a segmentation file" << std::endl
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (!std::filesystem::exists(p.i.bst_file)) {
        std::cout << std::endl
                  << "- Error: The required binary station file does not exist:" << std::endl
                  << "         " << p.i.bst_file << std::endl
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (!std::filesystem::exists(p.i.bms_file)) {
        std::cout << std::endl
                  << "- Error: The required binary measurement file does not exist:" << std::endl
                  << "         " << p.i.bms_file << std::endl
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (!userSuppliedSegFile) {
        if (std::filesystem::last_write_time(p.i.seg_file) < std::filesystem::last_write_time(p.i.bst_file) ||
            ...
            if ((bst_meta_import &&
                 (std::filesystem::last_write_time(p.i.seg_file) < std::filesystem::last_write_time(p.i.bst_file))) ||
                (bms_meta_import &&
                 (std::filesystem::last_write_time(p.i.seg_file) < std::filesystem::last_write_time(p.i.bms_file)))) {
                std::cout << std::endl
                          << std::endl
                ...
                std::cout << "   " << leafStr<std::string>(p.i.bms_file) << "  last modified on  " << ctime(&t_bms)
                          << std::endl;
                std::cout << "   " << leafStr<std::string>(p.i.seg_file) << "  created on  " << ctime(&t_seg)
                          << std::endl;
                std::cout << "  Run 'segment " << p.g.network_name
                          << " [options]' to re-create the segmentation file, or re-run" << std::endl
                          << "  the import using the " << SEG_FILE
                          << " option if this segmentation file must\n  be used." << std::endl
                          << std::endl;
                return EXIT_FAILURE;
            }
        }
...
int ImportSegmentedBlock(dna_import& parserDynaML, vdnaStnPtr* vStations, vdnaMsrPtr* vMeasurements,
    ...
    if (PrepareImportSegmentedData(p, userSuppliedSegFile) != EXIT_SUCCESS) return EXIT_FAILURE;

    // Import stations and measurements from a particular block
    if (!p.g.quiet)
        std::cout << std::endl
                  << "+ Importing stations and measurements from block " << p.i.import_block_number << " of\n  "
                  << p.i.seg_file << "... ";
    ...
    *parsemsrTally += parserDynaML.GetMsrTally();
    ...
    if (!userSuppliedSegFile) p.i.seg_file = "";
...
int ImportContiguousNetwork(dna_import& parserDynaML, vdnaStnPtr* vStations, vdnaMsrPtr* vMeasurements,
    ...
    if (!p.g.quiet)
        std::cout << std::endl
                  << "+ Importing stations and measurements from contiguous network " << p.i.import_network_number
                  << " of\n  " << p.i.seg_file << "... ";
...
int ImportDataFiles(dna_import& parserDynaML, vdnaStnPtr* vStations, vdnaMsrPtr* vMeasurements,
                    vdnaStnPtr* vstationsTotal, vdnaMsrPtr* vmeasurementsTotal, std::ofstream* imp_file,
                    vifm_t* vinput_file_meta, StnTally* parsestnTally, MsrTally* parsemsrTally, UINT32& errorCount,
                    project_settings& p) {
    // For consideration:
    ...
    size_t strlen_arg = 0;
    for_each(p.i.input_files.begin(), p.i.input_files.end(), [&strlen_arg](std::string& file) {
        if (leafStr<std::string>(file).length() > strlen_arg) strlen_arg = leafStr<std::string>(file).length();
    });
    ...
    if (!p.g.quiet) std::cout << "+ Parsing: " << std::endl;
    *imp_file << "+ Parsing " << std::endl;
    ...
    for (i = 0; i < nfiles; i++) {
        ...
        if (!p.g.quiet) std::cout << "  " << std::setw(strlen_arg) << std::left << ss;
        *imp_file << "  " << std::setw(strlen_arg) << std::left << ss;
        ...
        if (!p.g.quiet) ui_interop_threads.emplace_back(dna_import_progress_thread(&parserDynaML, &p));
        ui_interop_threads.emplace_back(dna_import_thread(&parserDynaML, &p, input_file, vStations, &stnCount,
                                                          vMeasurements, &msrCount, &clusterID, &input_file_meta,
                                                          firstFile, &status_msg, &elapsed_time));
        for (auto& t : ui_interop_threads) { t.join(); }
        ...
        case PARSE_UNRECOGNISED_FILE:
            *imp_file << status_msg << std::endl;
            errorCount++;
            continue;
        case PARSE_SUCCESS: running = false; break;
        default:
            errorCount++;
            std::cout << std::endl;
            continue;
        ...
        {
            ss_time << "  Done. Loaded ";
            if (stnCount) ss_time << stnCount << " stations";
            if (stnCount && msrCount) ss_time << " and ";
            if (msrCount) ss_time << msrCount << " measurements";
            ss_time << " in ";
            ...
            if (!p.g.quiet) {
                if (isatty(fileno(stdout))) std::cout << PROGRESS_BACKSPACE_04;
            ...
            *imp_file << time_message << std::endl;
            ...
            if (firstFile) {
                ...
                //  - a frame has not been supplied, in which case the file frame will be takenelse if
                //  (!p.i.user_supplied_epoch)
                ...
                if (p.i.import_block || p.i.import_network) continue;
                ...
                if (!p.i.user_supplied_frame) {
                    ...
                    case sinex: datumSource << ". DynAdjust default (frame not present within SNX file)"; break;
                    default:
                        if (parserDynaML.filespecifiedReferenceFrame())
                            datumSource << ". Taken from " << FormatFileType<std::string>(input_file_meta.filetype)
                                        << " header.";
                        else
                            datumSource << ". Reference frame not supplied in "
                                        << FormatFileType<std::string>(input_file_meta.filetype) << " header.";
                    }

                    if (!p.g.quiet)
                        std::cout << std::left << "  Project reference frame set to " << p.i.reference_frame
                                  << datumSource.str() << std::endl;
                    *imp_file << std::left << "  Project reference frame set to " << p.i.reference_frame
                              << datumSource.str() << std::endl;
                }
                // When the user has supplied a frame on the command line, and the input file datum field is blank
                    ...
                    case sinex:
                        ...
                        break;
                    default:
                        ssEpsgWarning << "  - Warning: Input file reference frame not supplied. Adopting "
                                      << inputFileDatum << ".";
                    }

                    if (!p.g.quiet) std::cout << ssEpsgWarning.str() << std::endl;
                    *imp_file << ssEpsgWarning.str() << std::endl;
                }
                // when reference frame has been supplied and the value in the file doesn't match
                    ...
                    if (!iequals(projectEpsgCode, input_file_meta.epsgCode)) {
                        std::stringstream ssEpsgWarning;
                        ssEpsgWarning << "  - Warning: File reference frame (" << inputFileDatum
                                      << ") differs from project frame (" << p.i.reference_frame << ").";
                        if (!p.g.quiet) std::cout << ssEpsgWarning.str() << std::endl;
                        *imp_file << ssEpsgWarning.str() << std::endl;
                    }
                }
                    ...
                    if (isStaticFrame) {
                        ...
                        if (parserDynaML.filespecifiedEpoch()) {
                            if (isEpsgDatumStatic(inputFileEpsgi))
                                ...
                                epochSource << ". Taken from " << FormatFileType<std::string>(input_file_meta.filetype)
                                            << " header.";
                        } else
                            epochSource << ". Epoch not supplied in "
                                        << FormatFileType<std::string>(input_file_meta.filetype) << " header.";
                        if (!p.g.quiet)
                            std::cout << std::left << "  Project epoch set to " << p.i.epoch << epochSource.str()
                                      << std::endl;
                        *imp_file << std::left << "  Project epoch set to " << p.i.epoch << epochSource.str()
                                  << std::endl;
                        epochChanged = true;
                    }
                }
                // When the user has supplied an epoch on the command line, and the input file epoch field is blank
                    ...
                    if (!p.g.quiet) std::cout << ssEpochWarning.str() << std::endl;
                ...
                // when epoch has been supplied and the value in the file doesn't match
                    ...
                    if (!iequals(p.i.epoch, input_file_meta.epoch)) {
                        std::stringstream ssEpochWarning;
                        ssEpochWarning << "  - Warning: File epoch (" << inputFileEpoch
                                       << ") differs from project epoch (" << p.i.epoch << ").";
                        if (!p.g.quiet) std::cout << ssEpochWarning.str() << std::endl;
                        *imp_file << ssEpochWarning.str() << std::endl;
                    }
                }
                ...
                if (referenceframeChanged || epochChanged) {
                    if (!p.g.quiet) std::cout << std::endl;
                    *imp_file << std::endl;
                }
            } else {
                if (p.i.import_block || p.i.import_network) continue;
                ...
                // Is the datum in the file different to the project datum?
                    ...
                    if (referenceframeChanged) {
                        ssEpsgWarning << "  - Warning: The project reference frame has been set to the default"
                                      << std::endl
                                      << "    file datum of " << leafStr<std::string>(p.i.input_files.at(i)) << " ("
                                      << inputFileDatum << ").";
                ...
                // Is the epoch in the file different to the project epoch?
                    ...
                    ssEpochWarning << "  - Warning: File epoch (" << inputFileEpoch << ") differs from project epoch ("
                                   << p.i.epoch << ").";
    ...
    if (errorCount == nfiles || (vstationsTotal->empty() && vmeasurementsTotal->empty())) return EXIT_FAILURE;
...
int main(int argc, char* argv[]) {
    ...
    boost::program_options::options_description ref_frame_options("+ " + std::string(IMPORT_MODULE_FRAME),
                                                                  PROGRAM_OPTIONS_LINE_LENGTH);
    boost::program_options::options_description data_screening_options("+ " + std::string(IMPORT_MODULE_SCREEN),
                                                                       PROGRAM_OPTIONS_LINE_LENGTH);
    boost::program_options::options_description gnss_scaling_options("+ " + std::string(IMPORT_MODULE_GNSS_VAR),
                                                                     PROGRAM_OPTIONS_LINE_LENGTH);
    boost::program_options::options_description simulation_options("+ " + std::string(IMPORT_MODULE_SIMULATE),
                                                                   PROGRAM_OPTIONS_LINE_LENGTH);
    boost::program_options::options_description output_options("+ " + std::string(ALL_MODULE_OUTPUT),
                                                               PROGRAM_OPTIONS_LINE_LENGTH);
    boost::program_options::options_description export_options("+ " + std::string(ALL_MODULE_EXPORT),
                                                               PROGRAM_OPTIONS_LINE_LENGTH);
    ...
    try {
        ...
        standard_options.add_options()(
            ...
            "Project file name. Full path to project file. If none specified, a new file is created using input-folder "
            "and network-name.  If a project file exists, the file is used to set all command line options.")(
            NETWORK_NAME_N, boost::program_options::value<std::string>(&p.g.network_name),
            "Network name. User defined name for all input and output files. Default is \"network#\", where # is "
            "incremented until the first available network is reached.")(
            ...
            IMPORT_GEO_FILE_G, boost::program_options::value<std::string>(&p.i.geo_file), "Import DNA geoid file.")(
            ...
            "Path for all output files.")(BIN_STN_FILE_S, boost::program_options::value<std::string>(&p.i.bst_file),
        ...
        ref_frame_options.add_options()(
            ...
            (std::string("Project reference frame for all stations, measurements, and preliminary reductions on the "
                         "ellipsoid when input files do not specify a reference frame. Default is ") +
             p.i.reference_frame + ".")
                .c_str())(EPOCH_E, boost::program_options::value<std::string>(&p.i.epoch),
        ...
        data_screening_options.add_options()(
            ...
            "Import stations and measurements within bounding box. arg is a comma delimited string "
            "\"lat1,lon1,lat2,lon2\" (in dd.mmss) defining the upper-left and lower-right limits.")(
            ...
            "Include stations and all associated measurements. arg is a comma delimited string \"stn 1,stn 2,stn "
            "3,...,stn N\" of the stations to include.")(
            EXCLUDE_STN_ASSOC_MSRS, boost::program_options::value<std::string>(&p.i.stn_associated_msr_exclude),
            "Exclude stations and all associated measurements. arg is a comma delimited string \"stn 1,stn 2,stn "
            "3,...,stn N\" of the stations to exclude.")(
            SPLIT_CLUSTERS,
            ...
            PREFER_X_MSR_AS_G, "Import single baseline cluster measurements (X) as single baselines (G).")(
            ...
            "Station renaming file")(STATION_DISCONTINUITY_FILE,
                                     boost::program_options::value<std::string>(&p.i.stn_discontinuityfile),
                                     "Station discontinuity file.  Applies discontinuity dates to station names in "
                                     "station and measurement files.")(TEST_NEARBY_STNS, "Search for nearby stations.")(
            ...
            (std::string("Specify the radius of the circle within which to search for nearby stations.  Default is ") +
             StringFromT(STN_SEARCH_RADIUS) + std::string("m"))
                .c_str())(TEST_SIMILAR_GNSS_MSRS,
                          "Search and provide warnings for GNSS baselines (G) and baseline clusters (X) which appear "
                          "to have been derived from the same source data.")(
            TEST_SIMILAR_MSRS, "Search and provide warnings for similar measurements.")(
            IGNORE_INSUFFICIENT_MSRS,
            "Ignore measurements which do not sufficiently constrain a station in two dimensions.")(
            IGNORE_SIMILAR_MSRS, "Ignore similar measurements.")(REMOVE_IGNORED_MSRS, "Remove ignored measurements.")(
            FLAG_UNUSED_STNS,
            "Mark unused stations in binary file.  Stations marked will be excluded from any further processing.")(
            TEST_INTEGRITY, "Test the integrity of the association lists and binary files.");

        gnss_scaling_options.add_options()(VSCALE, boost::program_options::value<double>(&p.i.vscale),
                                           (std::string("Global variance (v) matrix scalar for all GNSS measurements.  "
                                                        "Replaces existing scalar.  Default is ") +
                                            StringFromT(p.i.vscale) + std::string("."))
                                               .c_str())(
            PSCALE, boost::program_options::value<double>(&p.i.pscale),
            (std::string("Latitude (p=phi) variance matrix scalar for all GNSS measurements.  Replaces existing "
                         "scalar.  Default is ") +
             StringFromT(p.i.pscale) + std::string("."))
                .c_str())(LSCALE, boost::program_options::value<double>(&p.i.lscale),
                          (std::string("Longitude (l=lambda) variance matrix scalar for all GNSS measurements.  "
                                       "Replaces existing scalar.  Default is ") +
                           StringFromT(p.i.lscale) + std::string("."))
                              .c_str())(HSCALE, boost::program_options::value<double>(&p.i.hscale),
                                        (std::string("Height (h) variance matrix scalar for all GNSS measurements.  "
                                                     "Replaces existing scalar.  Default is ") +
                                         StringFromT(p.i.hscale) + std::string("."))
                                            .c_str())(
            SCALAR_FILE, boost::program_options::value<std::string>(&p.i.scalar_file),
            "File containing v, p, l and h scalars for GNSS baseline measurements between specific station pairs.  "
            "Scalar file values do not apply to GNSS point or baseline clusters.");
        ...
        export_options.add_options()(EXPORT_XML_FILES,
            ...
            EXPORT_SINGLE_XML_FILE, "Create a single DynaML file for stations and measurements.")
            ...
            (EXPORT_DNA_FILES, "Export stations and measurements to DNA STN and MSR format.")(
                EXPORT_ASL_FILE, "Export the ASL file as raw text.")(EXPORT_AML_FILE,
                                                                     "Export the AML file as raw text.")(
                EXPORT_MAP_FILE, "Export the MAP file as raw text.")(EXPORT_DISCONT_FILE,
                                                                     "Export discontinuity information as raw text.");

        simulation_options.add_options()(
            SIMULATE_MSR_FILE,
            "Simulate exact measurements corresponding to the input measurements using the coordinates in the station "
            "file. To apply geoid--ellipsoid separations and deflections of the vertical to the simulated "
            "measurements, introduce a geoid file using the --geo-file option.");
        ...
        allowable_options.add(standard_options)
            .add(ref_frame_options)
            .add(data_screening_options)
            .add(gnss_scaling_options)
    ...
    bool userSuppliedSegFile(false);
    if (!p.i.seg_file.empty()) userSuppliedSegFile = true;
    bool userSuppliedBstFile(false);
    if (!p.i.bst_file.empty()) userSuppliedBstFile = true;
    bool userSuppliedBmsFile(false);
    if (!p.i.bms_file.empty()) userSuppliedBmsFile = true;
    ...
    if (!p.g.quiet) {
        ...
        std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Associated station file: " << p.i.asl_file
                  << std::endl;
        std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Associated measurement file: " << p.i.aml_file
                  << std::endl;
        std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Binary station output file: " << p.i.bst_file
                  << std::endl;
        std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Binary measurement output file: " << p.i.bms_file
                  << std::endl;
        ...
        // in this case, it will be reported later
        if (p.i.user_supplied_frame)
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Project reference frame:" << p.i.reference_frame
                      << " (user supplied)" << std::endl;
        else
        ...
        if (isEpsgDatumStatic(epsgCode) && p.i.user_supplied_frame)
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Project epoch:" << p.i.epoch
                      << " (adopted reference epoch of " << p.i.reference_frame << ")" << std::endl;
        else {
            if (p.i.user_supplied_epoch)
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Project epoch:" << p.i.epoch
        ...
        if (p.i.export_dynaml) {
            if (p.i.export_single_xml_file)
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DynaML output file: " << p.i.xml_outfile
                          << std::endl;
            else {
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DynaML station file: " << p.i.xml_stnfile
                          << std::endl;
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DynaML measurement file: " << p.i.xml_msrfile
                          << std::endl;
            }
        }
        if (p.i.export_dna_files) {
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DNA station file: " << p.i.dna_stnfile
                      << std::endl;
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DNA measurement file: " << p.i.dna_msrfile
                      << std::endl;
        }

        if (p.i.simulate_measurements) {
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  DNA simulated msr file: " << p.i.simulate_msrfile
                      << std::endl;
        }
        ...
        if (p.i.import_block) {
            if (!p.i.seg_file.empty())
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Segmentation file: " << p.i.seg_file
                          << std::endl;
            std::cout << std::setw(PRINT_VAR_PAD) << std::left
                      << "  Import stns & msrs from block: " << p.i.import_block_number << std::endl;
        } else if (p.i.import_network) {
            if (!p.i.seg_file.empty())
                std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  Segmentation file: " << p.i.seg_file
                          << std::endl;
        ...
        if (!p.i.scalar_file.empty())
            std::cout << std::setw(PRINT_VAR_PAD) << std::left << "  GNSS baseline scalar file: " << p.i.scalar_file
                      << std::endl;
    ...
    if (vm.count(STATION_DISCONTINUITY_FILE)) {
        ...
        if (!std::filesystem::exists(p.i.stn_discontinuityfile)) {
            ...
            std::stringstream ss;
            ss << "- Warning: The station discontinuity file " << discontPath.filename().string()
               << " does not exist... ignoring discontinuity input." << std::endl;
    ...
    if (p.i.import_block) {
        imp_file << "+ Extracting stations and measurements from segmented block " << p.i.import_block_number << "... ";

        if (ImportSegmentedBlock(parserDynaML, &vstationsTotal, &vmeasurementsTotal, &parsestnTally, &parsemsrTally,
                                 p) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    ...
    // Import network information based on a contiguous network?
        ...
        imp_file << "+ Extracting stations and measurements from contiguous network " << p.i.import_network_number
                 << "... ";

        if (ImportContiguousNetwork(parserDynaML, &vstationsTotal, &vmeasurementsTotal, &parsestnTally, &parsemsrTally,
                                    p) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    ...
    if (p.i.ignore_insufficient_msrs == 1) {
        ...
        if (!p.g.quiet) {
            ...
            if (msrCount > vmeasurementsTotal.size())
                std::cout << "+ Removed " << (msrCount - vmeasurementsTotal.size())
                          << " measurements which alone do not sufficiently allow for" << std::endl
                          << "  the estimation of 2D coordinates." << std::endl;
        }
        imp_file << " Done. " << std::endl;
        if (msrCount > vmeasurementsTotal.size())
            imp_file << "+ Removed " << (msrCount - vmeasurementsTotal.size())
                     << " measurements which alone do not sufficiently allow for" << std::endl
                     << "  the estimation of 2D coordinates." << std::endl;
        ...
        if (!vPoorlyConstrainedStns.empty()) {
            if (vPoorlyConstrainedStns.size() == 1) {
                if (p.g.verbose > 2) {
                    if (!p.g.quiet)
                        std::cout << "- Warning: station " << vPoorlyConstrainedStns.at(0)
                                  << " is not associated with sufficient measurements." << std::endl;
                    imp_file << "- Warning: station " << vPoorlyConstrainedStns.at(0)
                             << " is not associated with sufficient measurements." << std::endl;
                } else {
                    if (!p.g.quiet)
                        std::cout << "- Warning: 1 station is not associated with sufficient measurements."
                                  << std::endl;
                ...
                if (p.g.verbose > 2) {
                    if (!p.g.quiet)
                        std::cout << "- Warning: The following " << vPoorlyConstrainedStns.size()
                                  << " stations are not associated with sufficient measurements." << std::endl;
                    imp_file << "- Warning: The following " << vPoorlyConstrainedStns.size()
                             << " stations are not associated with sufficient measurements." << std::endl;
                    _it_vstr poorly;
                    for (poorly = vPoorlyConstrainedStns.begin(); poorly != vPoorlyConstrainedStns.end(); poorly++) {
                        if (!p.g.quiet) outputObject(std::string("  - " + *poorly + "\n"), std::cout);
                        outputObject(std::string("  - " + *poorly + "\n"), imp_file);
                    }
                } else {
                    if (!p.g.quiet)
                        std::cout << "- Warning: " << vPoorlyConstrainedStns.size()
                                  << " stations are not associated with sufficient measurements." << std::endl;
                    imp_file << "- Warning: " << vPoorlyConstrainedStns.size()
                             << " stations are not associated with sufficient measurements." << std::endl;
                }
            }
        }
    ...
    // name.
    if (p.i.apply_discontinuities && !vstationsTotal.empty()) parserDynaML.AddDiscontinuityStations(&vstationsTotal);
    ...
    if (vstationsTotal.size() &&  // cannot be empty
        ...
        try {
            ...
            parserDynaML.ExtractStnsAndAssociatedMsrs(p.i.stn_associated_msr_include, p.i.stn_associated_msr_exclude,
                                                      &vstationsTotal, &vmeasurementsTotal, &parsestnTally,
                                                      &parsemsrTally, &vUnusedStns, p, splitXmsrs, splitYmsrs);
        } catch (const XMLInteropException& e) {
            std::cout << "- Error: " << e.what() << std::endl;
            imp_file << std::endl << "- Error: " << e.what() << std::endl;
            imp_file.close();
    ...
    if (vstationsTotal.size() &&  // cannot be empty
        ...
        try {
            ...
            parserDynaML.ExcludeAllOutsideBoundingBox(&vstationsTotal, &vmeasurementsTotal, &parsestnTally,
                                                      &parsemsrTally, &vUnusedStns, p, splitXmsrs, splitYmsrs);
        } catch (const XMLInteropException& e) {
            std::cout << "- Error: " << e.what() << std::endl;
            imp_file << std::endl << "- Error: " << e.what() << std::endl;
        ...
        if (!vUnusedStns.empty()) {
            ...
            ss_msg << " found outside the bounding box and " << (vUnusedStns.size() > 1 ? "have been" : "has been")
                   << std::endl
            ...
            if (!p.g.quiet) std::cout << ss_msg.str() << std::endl;

            for (stn = 0; stn < vUnusedStns.size(); ++stn) imp_file << "     " << vUnusedStns.at(stn) << std::endl;
            imp_file << std::endl;
        } else {
            imp_file << "  - No stations were found outside the bounding box." << std::endl;
            if (!p.g.quiet) std::cout << "  - No stations were found outside the bounding box." << std::endl;
        }
    }
    ...
    if ((stnCount + msrCount) > 0) {
        if (p.i.import_block || p.i.import_network) {
            if (!p.g.quiet) std::cout << "+ Binary file ";
            imp_file << "+ Binary file ";
        } else {
            if (!p.g.quiet) std::cout << "+ File ";
            imp_file << "+ File ";
        }

        if (!p.g.quiet) std::cout << "parsing summary:" << std::endl << std::endl;
    ...
    if (stnCount < 1) {
        ...
        if (!p.g.quiet) std::cout << "- No further processing can be done as no stations were loaded." << std::endl;
        imp_file.close();
    ...
    try {
        if (std::filesystem::exists(p.i.dst_file)) std::filesystem::remove(p.i.dst_file);
        if (std::filesystem::exists(p.i.dms_file)) std::filesystem::remove(p.i.dms_file);
    } catch (const std::ios_base::failure& f) {
    ...
    if (p.i.import_block == 1 || p.i.import_network == 1) {
        ...
        std::stringstream ss("");
        ss << formPath<std::string>(std::filesystem::path(p.i.bst_file).parent_path().generic_string(),
                                    std::filesystem::path(p.i.bst_file).stem().generic_string());
        ss << modifier.str() << ".bst";
        p.i.bst_file = ss.str();

        ss.str("");
        ss << formPath<std::string>(std::filesystem::path(p.i.bms_file).parent_path().generic_string(),
                                    std::filesystem::path(p.i.bms_file).stem().generic_string());
        ss << modifier.str() << ".bms";
        p.i.bms_file = ss.str();

        ss.str("");
        ss << formPath<std::string>(std::filesystem::path(p.i.asl_file).parent_path().generic_string(),
                                    std::filesystem::path(p.i.asl_file).stem().generic_string());
        ss << modifier.str() << ".asl";
        p.i.asl_file = ss.str();

        ss.str("");
        ss << formPath<std::string>(std::filesystem::path(p.i.aml_file).parent_path().generic_string(),
                                    std::filesystem::path(p.i.aml_file).stem().generic_string());
        ss << modifier.str() << ".aml";
        p.i.aml_file = ss.str();

        ss.str("");
        ss << formPath<std::string>(std::filesystem::path(p.i.map_file).parent_path().generic_string(),
                                    std::filesystem::path(p.i.map_file).stem().generic_string());
        ss << modifier.str() << ".map";
        p.i.map_file = ss.str();
        ...
        try {
            ...
            if (stn > 0) {
                ...
                if (!vduplicateStns.empty()) {
                    ss_msg.str("");
                    ss_msg << "Removed " << vduplicateStns.size() << " duplicate station"
                           << (vduplicateStns.size() > 1 ? "s" : "");
                    ...
                    imp_file << ss_msg.str();
                    if (!p.g.quiet) std::cout << std::endl << "- Warning: " << ss_msg.str();
                }

                if (!vnearbyStns.empty()) {
                    ss_msg.str("");
                    ss_msg << vnearbyStns.size()
                           << (vnearbyStns.size() > 1 ? " pairs of stations were" : " pair of station was")
                           << " found to be separated by less than " << std::setprecision(3) << p.i.search_stn_radius
                           << "m.";

                    imp_file << std::endl
                             << "- Warning: " << ss_msg.str() << std::endl
                             << "  See " << p.i.dst_file << " for details." << std::endl
                             << std::endl;
                    ...
                    dst_file << std::setw(HEADER_20) << std::left << "First station" << std::setw(HEADER_20)
                             << "Nearby station" << std::setw(HEADER_20) << std::right << "Separation (m)"
                             << std::setw(HEADER_20) << "Diff height (m)" << std::endl;

                    for (UINT32 i(0); i < (HEADER_20 * 4); ++i) dst_file << "-";
                    dst_file << std::endl;

                    // dump nearby stations to dst file
                    for (stn = 0; stn < vnearbyStns.size(); ++stn) {
                        dst_file << std::setw(HEADER_20) << std::left << vnearbyStns.at(stn).first.first <<  // First
                            std::setw(HEADER_20) << vnearbyStns.at(stn).first.second <<                      // Nearby
                            std::setw(HEADER_20) << std::setprecision(3) << std::fixed << std::right
                                 << vnearbyStns.at(stn).second.first <<  // Separation (m)
                            std::setw(HEADER_20) << std::setprecision(3) << std::fixed
                                 << vnearbyStns.at(stn).second.second <<  // Diff height (m)
                            std::endl;
                    }

                    ss_msg << "  If the names in each pair refer to the same station, then update the " << std::endl
                           << "  station and measurement files with the correct station name and re-run "
                           << __BINARY_NAME__ << "." << std::endl
                           << "  Alternatively, if the names in each pair are unique, either call " << __BINARY_NAME__
                           << std::endl
                           << "  without the --" << TEST_NEARBY_STNS << " option, or decrease the radial search "
                           << std::endl
                           << "  distance using --" << TEST_NEARBY_STN_DIST << "." << std::endl
                           << std::endl;
    ...
    if (msrCount > 0 && (p.i.search_similar_msr || p.i.search_similar_msr_gx || p.i.ignore_similar_msr)) {
        if (SearchForSimilarMeasurements(&parserDynaML, &p, &imp_file, &vmeasurementsTotal) != EXIT_SUCCESS)
            return EXIT_FAILURE;
    }
    ...
    if (stn_map_created) {
        try {
            ...
            if (msrCount > 0) {
                ...
                imp_file << "Mapped " << mapCount << " measurements to "
                         << vStnsMap_sortName.size() - vunusedStations.size() << " stations." << std::endl;
            } else
                imp_file << std::endl;
            ...
            if (!vunusedStations.empty()) {
                if (vunusedStations.size() == 1) {
                    if (p.g.verbose > 2) {
                        if (!p.g.quiet)
                            std::cout << "- Warning: station " << vunusedStations.at(0)
                                      << " was not associated with any measurements." << std::endl;
                        imp_file << "- Warning: station " << vunusedStations.at(0)
                                 << " was not associated with any measurements." << std::endl;
                    } else {
                        if (!p.g.quiet)
                            std::cout << "- Warning: " << vunusedStations.size()
                                      << " station was not associated with any measurements." << std::endl;
                        imp_file << "- Warning: " << vunusedStations.size()
                                 << " station was not associated with any measurements." << std::endl;
                    }
                } else {
                    if (p.g.verbose > 2) {
                        if (!p.g.quiet)
                            std::cout << "- Warning: The following " << vunusedStations.size()
                                      << " stations were not associated with any measurements." << std::endl;
                        imp_file << "- Warning: The following " << vunusedStations.size()
                                 << " stations were not associated with any measurements." << std::endl;
                        _it_vstr unused;
                        for (unused = vunusedStations.begin(); unused != vunusedStations.end(); unused++) {
                            if (!p.g.quiet) outputObject(std::string("  - " + *unused + "\n"), std::cout);
            ...
            if (msrRead < mapCount && vignoredMeasurements.empty()) {
                if (!p.g.quiet)
                    std::cout << "- Warning: Not all measurements were mapped: " << msrRead << " msrs read vs. "
                              << mapCount << " msrs mapped." << std::endl;
                ...
                if (!p.g.quiet) {
                    ...
                    if ((msrRead - mapCount) != ignMsrCount)
                        std::cout << "-          " << msrRead << " m.read vs. " << mapCount << " m.mapped."
                                  << std::endl;
                }
            }
            if (!p.g.quiet) std::cout.flush();
        } catch (const XMLInteropException& e) {
            std::cout << std::endl << "- Error: " << e.what() << std::endl;
            imp_file << std::endl << "- Error: " << e.what() << std::endl;
            imp_file.close();
    ...
    if (stnCount > static_cast<UINT32>(associatedSL.size())) stnCount = static_cast<UINT32>(associatedSL.size());
    ...
    if (p.i.simulate_measurements && stnCount > 0 && msrCount > 0) {
        ...
        try {
            ...
            imp_file << "+ Simulating and exporting measurements to " << leafStr<std::string>(p.i.simulate_msrfile)
                     << "... ";
            parserDynaML.SimulateMSR(((vdnaStnPtr*)&vstationsTotal), ((vdnaMsrPtr*)&vmeasurementsTotal),
    ...
    if (stnCount > 0) {
        try {
            if (!vstationsTotal.empty()) {
                ...
                parserDynaML.SerialiseBst(p.i.bst_file, ((vdnaStnPtr*)&vstationsTotal), &vunusedStations,
                                          vinput_file_meta, (p.i.flag_unused_stn ? true : false));
                if (!p.g.quiet) {
                    std::cout << "Done." << std::endl;
                    std::cout.flush();
    ...
    if (msrCount > 0) {
        try {
            if (!vmeasurementsTotal.empty()) {
                ...
                parserDynaML.SerialiseBms(p.i.bms_file, ((vdnaMsrPtr*)&vmeasurementsTotal), vinput_file_meta);
                if (!p.g.quiet) {
                    std::cout << "Done." << std::endl;
                    std::cout.flush();
                }
    ...
    if (measurements_mapped) {
        ...
        try {
            if (p.i.export_aml_file) {
                ...
                parserDynaML.SerialiseAmlTextFile(p.i.bms_file, p.i.aml_file, &associatedML, &associatedSL,
        ...
        if (p.i.test_integrity) {
            ...
            std::ifstream binaryMS(p.i.bms_file.c_str(),
                                   std::ios::in | std::ios::binary |
                                       std::ifstream::ate);  /// Open and seek to end immediately after opening.
            if (!binaryMS.good()) {
            ...
            for (stn = 0; stn < stnCount; stn++) {
                ...
                for (msr = 0; msr < amlcount; msr++) {
                    ...
                    if (associatedML.at(amlindex) * sizeof(measurement_t) >= sFileSize) {
                        std::cout << "Error: index " << associatedML.at(amlindex)
                                  << " is out of range for the binary file." << std::endl;
                        continue;
                    }
            ...
            binaryMS.close();
            if (!p.g.quiet) std::cout << "OK." << std::endl;
        }
    }
    ...
    try {
        if (p.i.export_dynaml || p.i.export_dna_files)
            ExportStationsandMeasurements(&parserDynaML, p, &imp_file, &vinput_file_meta, &vstationsTotal,
                                          &vmeasurementsTotal, stnCount, msrCount);
    } catch (const XMLInteropException& e) {
        std::cout.flush();
        std::cout << std::endl << "- Error: " << e.what() << std::endl;
        imp_file << std::endl << "- Error: " << e.what() << std::endl;
    ...
    if (!userSuppliedSegFile) p.i.seg_file = "";
    if (!userSuppliedBstFile) p.i.bst_file = "";
    if (!userSuppliedBmsFile) p.i.bms_file = "";
    ...
    if (errorCount) {
        if (!p.g.quiet)
            std::cout << "- Warning: some files were not parsed - please read the log file for more details."
                      << std::endl;
    ...
    if (isEpsgWGS84Ensemble(epsgCode)) {
        ...
        ssEnsembleWarning << std::endl
                          << "- Warning:  The '" << p.i.reference_frame
                          << "' reference frame set for this project refers to the" << std::endl
        ...
        if (!p.g.quiet) std::cout << ssEnsembleWarning.str();
        imp_file << ssEnsembleWarning.str();
    }

    boost::posix_time::milliseconds elapsed_time(
    ...
    if (!p.g.quiet) std::cout << std::endl << time_message << std::endl;
    imp_file << std::endl << time_message << std::endl;

    if (stnCount > 0 && msrCount > 0) {
        if (!p.g.quiet)
            std::cout << "+ Binary station and measurement files are now ready for processing." << std::endl
                      << std::endl;
...
dna_import_thread::dna_import_thread(dynadjust::dynamlinterop::dna_import* dnaParse, project_settings* p,
                                     const std::string& filename, vdnaStnPtr* vStations, PUINT32 stnCount,
                                     vdnaMsrPtr* vMeasurements, PUINT32 msrCount, PUINT32 clusterID,
                                     input_file_meta_t* input_file_meta, bool firstFile, std::string* status_msg,
    ...
    : _dnaParse(dnaParse),
      ...
      _stnCount(stnCount),
      _vMeasurements(vMeasurements),
      _msrCount(msrCount),
      _clusterID(clusterID),
      _input_file_meta(input_file_meta),
      _firstFile(firstFile),
      _status_msg(status_msg),
      _ms(ms) {}

void dna_import_thread::operator()() {
    cpu_timer time;  // constructor of cpu_timer calls start()
    try {
        _dnaParse->ParseInputFile(_filename, _vStations, _stnCount, _vMeasurements, _msrCount, _clusterID,
                                  _input_file_meta, _firstFile, _status_msg, _p);
        *_ms = boost::posix_time::milliseconds(
            std::chrono::duration_cast<std::chrono::milliseconds>(time.elapsed().wall).count());
    } catch (const XMLInteropException& e) {
        running = false;
...
void dna_import_thread::SetFile(const std::string& file) { _filename = file; }

dna_import_progress_thread::dna_import_progress_thread(dynadjust::dynamlinterop::dna_import* dnaParse,
                                                       project_settings* p)
    : _dnaParse(dnaParse), _p(p) {}

void dna_import_progress_thread::operator()() {
